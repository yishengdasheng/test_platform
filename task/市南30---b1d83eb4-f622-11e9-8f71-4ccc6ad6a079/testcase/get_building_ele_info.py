...
class get_building_ele_info(unittest.TestCase):
    '''查询整栋建筑用电量'''
    @classmethod
    def setUpClass(cls):
        ...
        api="/demo/get_building_ele_info"
    ...
    def test_查询建筑用电量_馨逸公寓(self):
        CarryTask.objects.filter(id=self.transferlogname.test_carryid).update(stepcountnow=F('stepcountnow')+1)
        '''查询馨逸公寓总用电量和最近一个月用电量'''
        step_name="查询建筑用电量_馨逸公寓"
        newVariableObj = {}
        sqlDatalist=[]
        nosqlDatalist=[]
        api_dependency={}
        ...
        # 后置sql的执行
        carry_sql(self.transferip_db,self.transferlogname.test_carryid,self.transferfunction,sqlDatalist, 2,newVariableObj)
    def test_查询建筑用电量_不存在的建筑id(self):
        CarryTask.objects.filter(id=self.transferlogname.test_carryid).update(stepcountnow=F('stepcountnow')+1)
        ''''''
        step_name="查询建筑用电量_不存在的建筑id"
        newVariableObj = {}
        sqlDatalist=[]
        nosqlDatalist=[]
        api_dependency={}
        ...
        # 后置sql的执行
        carry_sql(self.transferip_db,self.transferlogname.test_carryid,self.transferfunction,sqlDatalist, 2,newVariableObj)
    def test_查询建筑用电量_id不是数字(self):
        CarryTask.objects.filter(id=self.transferlogname.test_carryid).update(stepcountnow=F('stepcountnow')+1)
        ''''''
        step_name="查询建筑用电量_id不是数字"
        newVariableObj = {}
        sqlDatalist=[]
        nosqlDatalist=[]
        api_dependency={}
